# Downloads - WHO SMART DAK — Cervical Cancer Screening (L2) v0.1.0
...
* **[purposive.dmn](l2/purposive.dmn)** — Purposive encoding with interpretive resolutions from the register. 6 decisions, 42 rules. Resolves screening intervals to point estimates per [cc-005](cc-005.md), adds pregnancy handling per [cc-006](cc-006.md).
...
1. POST to the decision evaluation endpoint with the decision ID and input variables
...
Decision IDs follow the convention `CC.<activity>.<name>_{t,p}` where `_t` denotes textualist and `_p` purposive. See the [Decision Logic](decision-logic.md) page for the full list.
