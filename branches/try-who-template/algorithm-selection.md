# Algorithm Selection Analysis - WHO SMART DAK — Cervical Cancer Screening (L2) v0.1.0
...
**Date:** 2026-04-13 **Authors:** Dan Heslinga, with Claude (Anthropic) **Status:** Draft
...
This document supports the scope declaration in [`../interpretation-register/cc-000-scope-declaration.md`](cc-000.md), which discloses the choice of Algorithm 5 and its biases. That entry summarises; this document shows the work.
...
1. For WLHIV, use triage (conditional recommendation, directional).
1. The triage method is a country decision based on feasibility and resources (conditional recommendation, explicitly non-prescriptive).
